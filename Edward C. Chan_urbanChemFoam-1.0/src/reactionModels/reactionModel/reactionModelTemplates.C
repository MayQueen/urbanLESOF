...
Foam::autoPtr<CombustionModel> Foam::reactionModel::New
(
...
)
{
    ...
    {
        IOdictionary(combIO).lookup("reactionModel") >> combModelName;
    }
    else
    {
    ...
    Info<< "Selecting reaction model " << combModelName << endl;
    ...
    if (cmpts2.size() == 2 || cmpts3.size() == 3)
    {
        ...
        WarningInFunction
            ...
            << "reaction model " << combModelName << "." << endl;
    }
    ...
    (
      ...
      ? thermoCstrIter()(combModelName, thermo, turb, reactionProperties)
      : compCstrIter()(combModelName, thermo, turb, reactionProperties)
    );
}
